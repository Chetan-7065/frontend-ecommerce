...
export function EcommerceProvider({ children }) {
  ...
  const [primaryAddress, setPrimaryAddress] = useState(
    userDetails.addresses.find((address) => address.isPrimary),
  );
  ...
  function updateCartList(productId, productTitle) {
    ...
    if (existingProduct) {
      ...
      product = [
        ...cartList,
        { id: productId, title: productTitle, quantity: 1 },
      ];
    }
  ...
  function increaseQuantity(productId) {
    ...
    if (product.quantity >= 10) {
      toast.error(`${product.title} reached out max quantity`, {
        toastId: "max-qty-toast",
      });
    } else {
      toast.success(`${product.title} quantity increased`);
    }
    const updateProduct = cartList.map((product) =>
      product.id === productId && product.quantity < 10
        ? { ...product, quantity: product.quantity + 1 }
        : product,
    );
  ...
  function decreaseQuantity(productId) {
    ...
    const filteredProduct = updateProduct.filter(
